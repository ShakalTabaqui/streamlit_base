...
st.write("""
# Приложение, выполняющее логарифмическую регрессию
""")

st.sidebar.header('Пользовательская настройка')
st.sidebar.write("""
# Dataset должен быть оцифрован. Обязательно укажите нормирован ли Датасет.
Таргет должен быть последней колонкой.
""")

st.sidebar.header('Ваш Датасет нормирован?')
norm = st.sidebar.selectbox('Выбери',('Да','Нет'))

uploaded_file = st.sidebar.file_uploader('Перетащите сюда Ваш Dataset', type=['csv'])
uploaded_df = pd.read_csv(uploaded_file)

st.sidebar.header('Выберите кол-во итераций')
n_inputs = st.sidebar.slider('Количество итераций', 20,100000,1000)

st.sidebar.header('Выберите learning rate')
